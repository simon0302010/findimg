...
impl FromIterator<(TodoStatus, &'static str, &'static str)> for TodoList {
    fn from_iter<I: IntoIterator<Item = (TodoStatus, &'static str, &'static str)>>(
        iter: I,
    ) -> Self {
        let items = iter
            .into_iter()
...
impl TodoItem {
    fn new(status: TodoStatus, todo: &str, info: &str) -> Self {
        Self {
            status,
            todo: todo.to_string(),
            info: info.to_string(),
...
impl From<&TodoItem> for ListItem<'_> {
    fn from(value: &TodoItem) -> Self {
        let line = match value.status {
            TodoStatus::Todo => Line::styled(format!(" ☐ {}", value.todo), TEXT_FG_COLOR),
